...
CI: a `fleetwatch` job in `.github/workflows/build.yml` calling the shared `docker.yml`
(path `code/kubes/fleetwatch`, image `fleetwatch`), gated on verify jobs like messages.
...
wrapper at `code/kubes/fleetwatch/android/` (`org.xinutec.fleetwatch`, hardcoded

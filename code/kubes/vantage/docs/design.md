# vantage — fleet monitoring platform
...
wrapper) is live on isis at `https://vantage.xinutec.org` (VPN-only, TLS issued),
...
- **The platform (`vantage`)** — one k3s service on isis that ingests reports, stores
...
- No alerting / push notifications. It is a pull-based dashboard; dead-man alerting
  stays on healthchecks.io. (Future: vantage itself could notify, or expose its own
...
config. A source that has *never* reported is unknown to vantage; the eventual
...
Dockerfile → `xinutec/vantage:latest`, nonroot 65532, read-only rootfs, tight
requests/limits).

k8s (`code/kubes/vantage/k8s/`, numbered like life):
...
- **VPN-only** exactly like messages: DNS `A vantage → 10.100.0.2`
...
- `whitelist-source-range: "10.100.0.0/24"` was TRIED on deploy (2026-07-03) and
  ...
  nginx sees it, so the rule 403s even a legit VPN client. So vantage runs at
...
**Ingest tokens**: env var in the k8s secret, `VANTAGE_TOKENS="mac-mini:<random>"`
...
Mac the token lives in a `0600` file under `~/.config/vantage/`, read by the pusher
...
CI: a `vantage` job in `.github/workflows/build.yml` calling the shared `docker.yml`
(path `code/kubes/vantage`, image `vantage`), gated on verify jobs like messages.
...
### 7.2 `vantage_push.py` (spool + upload)

```
vantage_push.py run <collector> -- <command...>   # run tool, capture JSON, spool, flush
vantage_push.py flush                             # retry everything in the spool
```

- Runs the collector with `--json`, stamps `id` (ULID) + `duration_ms`, writes the
  report to `~/.local/state/vantage/spool/<id>.json`, then attempts upload of every
  spooled file to `https://vantage.xinutec.org/api/reports`; deletes on 2xx
...
One launchd plist per cadence invoking `vantage_push.py run …`. **Known risk to
...
wrapper at `code/kubes/vantage/android/` (`org.xinutec.vantage`, hardcoded
`VANTAGE_URL`, recall `#android` nix shell, `deploy.sh` sideload) — identical to
life's wrapper.
...
1. **Platform skeleton** — `code/kubes/vantage/` cloned from life: schema,
   ...
   Deployed and accepting `curl` reports. Add `vantage` to `~/Code/check` REPOS.
2. **Mac producer** — `_checks.py --json`, `fleet.py` stable keys + `--json`,
   `vantage_push.py`, launchd timers. Real data flowing.

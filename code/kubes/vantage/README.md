...
vantage stores their history and serves a mobile UI. Adding a producer needs zero
code here. VPN-only on isis (`vantage.xinutec.org` → `10.100.0.2`), no login.

Design: [`docs/design.md`](docs/design.md).

## Stack

Rust axum + MariaDB + Angular 22 (zoneless), one image (`xinutec/vantage:latest`)
...
- `frontend/` — the Angular app (`vantage-web`).
- `android/` — single-WebView wrapper (`org.xinutec.vantage`).
...
export VANTAGE_TOKENS=mac-mini:dev-token
export DATABASE_URL=mysql://vantage:vantage@127.0.0.1:3307/vantage
...
VANTAGE_TEST_DATABASE_URL=$DATABASE_URL nix develop -c cargo test   # incl. DB tests
./scripts/verify.sh                                   # full gate
```

## Deploy (isis)

CI publishes `xinutec/vantage:latest` on push to main. Then, on isis:
...
1. One-time: `letsencrypt-dns` issuer + `cloudflare-api-token` (shared with
   ...
   copy each printed token to its producer's `~/.config/vantage/token`); the DNS A
   record `vantage → 10.100.0.2` (`code/dns`, `tofu apply`).
2. `sudo ./k8s/sync.sh`.

...
#[derive(Clone, Debug)]
pub struct Config {
    /// MariaDB connection string, e.g. `mysql://vantage:pw@host/vantage`.
...
/// Parse `VANTAGE_TOKENS` — a comma-separated list of `source:token` pairs, e.g.
...
pub fn parse_tokens(raw: &str) -> Result<Vec<(String, String)>> {
    ...
    for entry in raw.split(',') {
        ...
        let (source, token) = entry
            .split_once(':')
            .with_context(|| format!("VANTAGE_TOKENS entry {entry:?} is not source:token"))?;
        let source = source.trim();
        let token = token.trim();
        if source.is_empty() || token.is_empty() {
            anyhow::bail!("VANTAGE_TOKENS entry {entry:?} has an empty source or token");
        }
...
impl Config {
    pub fn from_env() -> Result<Self> {
        let tokens = parse_tokens(&env("VANTAGE_TOKENS")?)?;
        if tokens.is_empty() {
            anyhow::bail!("VANTAGE_TOKENS is empty — no producer could ever authenticate");
        }
        Ok(Self {
            ...
            tokens,
            raw_retention_days: env_i64("VANTAGE_RAW_RETENTION_DAYS", 30)?,
            check_retention_days: env_i64("VANTAGE_CHECK_RETENTION_DAYS", 400)?,
        })
    }
}

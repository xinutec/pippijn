...
The `home.xinutec.org` environment dashboard presented as a native-feeling app: a
single full-screen **WebView**, no address bar, no tabs, a home-screen icon. It
avoids browser chrome while showing the dashboard exactly as designed (the system
WebView is Chromium, so it renders like Chrome).

The dashboard is **public over HTTPS** (reads need no auth), so the app needs only
`INTERNET` — no cleartext, no login handling.

## What it does

- Loads `https://home.xinutec.org/` — **hardcoded** (`MainActivity.HOME_URL`); this
...
"$ADB" -s 192.168.1.133:5555 shell am start -n org.xinutec.home/.MainActivity
```
...
│       ├── kotlin/org/xinutec/home/MainActivity.kt  # the WebView
│       └── res/                                 # launcher icon (green eco leaf), theme, strings

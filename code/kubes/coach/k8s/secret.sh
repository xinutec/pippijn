#!/usr/bin/env bash
# Run once on isis (as root) to create the coach k8s secret. Generates the DB
# and session secrets; the Nextcloud OAuth2 client values are passed in:
#
#   NC_CLIENT_ID=... NC_CLIENT_SECRET=... ./secret.sh
#
# (From Nextcloud admin → Settings → Security → OAuth 2.0, client "coach",
#  redirect https://coach.xinutec.org/auth/callback.)
set -euo pipefail
...
kubectl create secret -n coach generic coach-secret \
  --from-literal=DB_USER=coach \
  --from-literal=DB_PASSWORD="$DB_PASSWORD" \
  --from-literal=DB_ROOT_PASSWORD="$DB_ROOT_PASSWORD" \
  --from-literal=DATABASE_URL="mysql://coach:${DB_PASSWORD}@coach-db/coach" \

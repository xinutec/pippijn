 ...
 *  vertex — a fix map-matched onto a rail track rather than a measured
 *  GPS position. Inferred runs are drawn dashed. */
...
export class MapComponent implements OnDestroy {
	...
	private render(el: HTMLElement, data: VelocityData | null, fix: LatestFix | null): void {
		...
		for (const seg of segments) {
			...
			// A train run on an identified line carries a snappedPath —
			// its raw fixes map-matched onto the line's OSM track. Draw
			// that inferred geometry in place of the raw zigzag. It can
			// span a GPS-dark window with no Kalman points of its own,
			// so this branch comes before the inSeg emptiness check.
		...
		// run's last point so the line is continuous across the change.
		// Inferred (snapped) runs are drawn dashed to read as inferred.
		let runStart = 0;

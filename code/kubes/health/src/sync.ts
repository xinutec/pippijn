...
import { initPool, db, withConnection, destroyPool } from "./db/pool.js";
...
import { syncHeartRateZones, syncHeartRateIntraday } from "./fitbit/sync/heartrate.js";
  ...
  try {
    ...
    await withConnection(async (conn) => {
      ...
      for (const fn of optional) {
        try { await fn(); }
        catch (e) { console.log(`[${user.user_id}] Optional sync skipped: ${e}`); }
      }
    });

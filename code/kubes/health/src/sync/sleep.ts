...
interface SleepResponse { sleep: SleepLog[]; }
...
export async function syncSleep(
  ...
  for (const log of data.sleep) {
    await db.query(
      `INSERT INTO sleep
        (user_id, log_id, date, start_time, end_time, duration_ms, efficiency,
      ...
      [userId, log.logId, log.dateOfSleep, log.startTime, log.endTime,
       ...
       log.levels?.summary.wake?.minutes ?? null, log.isMainSleep]
    );
      ...
      for (const stage of log.levels.data) {
        await db.query(
          `INSERT INTO sleep_stages (user_id, sleep_log_id, ts, stage, duration_seconds)
           VALUES (?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE stage = VALUES(stage), duration_seconds = VALUES(duration_seconds)`,
  ...
  console.log(`[${userId}] Synced ${synced} sleep logs`);
  return synced;
}

  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    await db.query(
      `INSERT INTO daily_activity
        (user_id, date, steps, calories_total, calories_active, distance_km,
         floors, elevation_m, minutes_sedentary, minutes_lightly_active,
         minutes_fairly_active, minutes_very_active, active_score,
         resting_heart_rate)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         steps = VALUES(steps), calories_total = VALUES(calories_total),
         calories_active = VALUES(calories_active), distance_km = VALUES(distance_km),
         floors = VALUES(floors), elevation_m = VALUES(elevation_m),
      ...
      [userId, dateStr, s.steps, s.caloriesOut, s.activityCalories,
       totalDistance, s.floors, s.elevation, s.sedentaryMinutes,
       s.lightlyActiveMinutes, s.fairlyActiveMinutes, s.veryActiveMinutes,
       s.activeScore, s.restingHeartRate ?? null]
    );
    synced++;

    if (client.rateLimitRemaining <= 10) {
      console.log(`Activity sync paused at ${dateStr}, rate limit low (${client.rateLimitRemaining})`);
      break;
    }
  }

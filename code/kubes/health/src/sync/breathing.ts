...
export async function syncBreathingRate(
  ...
  for (const entry of data.br) {
    const v = entry.value;
    await db.query(
      `INSERT INTO breathing_rate
        (date, full_sleep_rate, deep_sleep_rate, light_sleep_rate, rem_sleep_rate)
       VALUES (?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         full_sleep_rate = VALUES(full_sleep_rate),
         deep_sleep_rate = VALUES(deep_sleep_rate),
         light_sleep_rate = VALUES(light_sleep_rate),
         rem_sleep_rate = VALUES(rem_sleep_rate)`,
      [
        entry.dateTime,
        v.fullSleepSummary?.breathingRate ?? v.breathingRate,
        v.deepSleepSummary?.breathingRate ?? null,
        v.lightSleepSummary?.breathingRate ?? null,
  ...
  console.log(`Synced ${synced} days of breathing rate data`);
  return synced;
}

...
| `2026-06-pedestrian-trajectory-smoother.md` | **shipped (display layer)** | The walking arm of map-constrained positioning. A MAP factor-graph smoother fusing robust GPS + pedometer distance (PDR) + endpoint anchors + gait smoothness + a *soft, openness-modulated* walkable-surface prior (≈0 in a park/forest — "free to walk anywhere"). Each vertex carries a posterior σ. `pedestrian-smooth.ts` + `annotateWalkSmoothing` pass; display-only (`kind:"smoothed"`), self-checking improvement gate. Measured: 2026-06-21 Varley walks step-error 110%→5% / 105%→6% |

...
in  { name = "ircd"
    ...
    , acme = Some
      ...
      , -- ⚠ **THIS DOES NOT DEFINE A CERTIFICATE.** `security.acme` on the host
        -- issues this name (#1294); the field decides which socket serves it.
        -- Rendering a `cert-manager.io/cluster-issuer` annotation here would make
        -- this file the SOLE definition of `irc-tls` again, and a stale issuer in
        -- it reissues IRC's certificate from the wrong CA on the next apply,
        -- breaking TLS for every connected client.

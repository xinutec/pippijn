...
// leave unrouted. Matches life/fleetwatch's e2e/serve.mjs.
import { createServer } from 'node:http';
import { readFile, stat } from 'node:fs/promises';
import { extname, join, normalize } from 'node:path';
import { fileURLToPath } from 'node:url';

const ROOT = join(fileURLToPath(new URL('.', import.meta.url)), '..', 'dist', 'messages-web', 'browser');
...
// backend. Signed-in user + no conversations, so an un-mocked run still renders.
const API = {

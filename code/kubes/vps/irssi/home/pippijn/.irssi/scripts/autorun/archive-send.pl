...
our %IRSSI = (
    ...
    # about something else. Since dev-lint 330c392 the rule knows that itself:
    # a match inside the repository's own `git config user.name` is not a leak,
    # because git writes that name into every commit it makes. The waiver this
    # used to carry suppressed nothing once that landed, so it is gone.

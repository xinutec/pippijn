//! pulse — fleet monitoring platform backend. Entry point: load config, connect
//! the DB, run migrations, start the retention sweeper, serve. All logic lives
//! in the `pulse` library crate.

use anyhow::Result;
use pulse::{config::Config, db, report::retention, routes, state::AppState};
...
async fn main() -> Result<()> {
    ...
    tracing::info!("pulse listening on {bind_addr}");
